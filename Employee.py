...
def Database():
    conn = sqlite3.connect("C:/Users/Admin/Desktop/pythontut.db")
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `member` (mem_id INTEGER NOT NULL  PRIMARY KEY AUTOINCREMENT, firstname TEXT, lastname TEXT, education TEXT, gender TEXT, age TEXT, address TEXT, contact TEXT)")
...
def SubmitData():
    if  FIRSTNAME.get() == "" or LASTNAME.get() == "" or EDUCATION.get()== "" or GENDER.get() == "" or AGE.get() == "" or ADDRESS.get() == "" or CONTACT.get() == "":
        result = tkMessageBox.showwarning('', 'Please Complete The Required Field', icon="warning")
    else:
        tree.delete(*tree.get_children())
        conn = sqlite3.connect("pythontut.db")
        cursor = conn.cursor()
        cursor.execute("INSERT INTO `member` (firstname, lastname, education, gender, age, address, contact) VALUES(?, ?, ?, ?, ?, ?, ?)", (str(FIRSTNAME.get()), str(LASTNAME.get()),str(EDUCATION.get()), str(GENDER.get()), int(AGE.get()), str(ADDRESS.get()), str(CONTACT.get())))
        conn.commit()
        cursor.execute("SELECT * FROM `member` ORDER BY `lastname` ASC")
        fetch = cursor.fetchall()
        for data in fetch:
...
def UpdateData():
    if GENDER.get() == "":
       result = tkMessageBox.showwarning('', 'Please Complete The Required Field', icon="warning")
        ...
        conn = sqlite3.connect("pythontut.db")
        cursor = conn.cursor()
        cursor.execute("UPDATE `member` SET `firstname` = ?, `lastname` = ?, `education` = ?, `gender` = ? ,`age` = ?,  `address` = ?, `contact` = ? WHERE `mem_id` = ?", (str(FIRSTNAME.get()), str(LASTNAME.get()), str(EDUCATION.get()), str(GENDER.get()), str(AGE.get()), str(ADDRESS.get()), str(CONTACT.get()), int(mem_id)))
        conn.commit()
        cursor.execute("SELECT * FROM `member` ORDER BY `lastname` ASC")
        fetch = cursor.fetchall()
        for data in fetch:
...
def OnSelected(event):
    ...
    #===================LABELS==============================
    lbl_title = Label(FormTitle, text="Updating Employee Information", font=('arial', 16), bg="Red",  width = 300)
    ...
    lbl_education = Label(ContactForm, text="Education", font=('arial', 14), bd=5)
    lbl_education.grid(row=2, sticky=W)
    ...
    education = Entry(ContactForm, textvariable=EDUCATION, font=('arial', 14))
...
#============================LABELS======================================
lbl_title = Label(Top, text="Employee Management System", font=('arial', 16), width=500)
lbl_title.pack(fill=X)
...
tree = ttk.Treeview(TableMargin, columns=("MemberID", "Firstname", "Lastname","Education", "Gender", "Age", "Address", "Contact"), height=400, selectmode="extended", yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
